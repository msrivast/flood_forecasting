...
r = np.random.choice(a, 126, replace=False) #608 for 7 hours and >0.2 rainfall
...
df = df.dropna()
plt.twinx().plot(1200-df['level'],c='magenta', alpha=0.7)
plt.twinx().plot(1200-df_curated[df_curated.valid]['level'], c='green', alpha=0.7)
plt.title("WS#2(15min) vs NHC_296(min) 4-hr time to concentration(May)")
plt.ylabel("Level rise (mm)")
plt.show()

# df_curated.to_csv('NHC7_curated_15m_7_past_12_test_large_rain.csv',float_format='%.3f')
print(df_curated)